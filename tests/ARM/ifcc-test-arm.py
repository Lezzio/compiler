...
orig_cwd=os.getcwd()
if "ifcc-arm-test-output" in orig_cwd:
    print('error: cannot run from within the output directory')
    exit(1)
    
if os.path.isdir('ifcc-arm-test-output'):
    # cleanup previous output directory
    command('rm -rf ifcc-arm-test-output')
os.mkdir('ifcc-arm-test-output')
...
for inputfilename in inputfilenames:
    ...
    if 'ifcc-arm-test-output' in os.path.realpath(inputfilename):
    ...
    subdir='ifcc-arm-test-output/'+inputfilename.strip("./")[:-2].replace('/','-')
    os.mkdir(subdir)
